...
elseif strcmp(dataset,'#6-California') == 1 % Heterogeneous CD of SAR VS. Optical
    ...
    subplot(133);imshow(Ref_gt,[]);title('Refgt')
elseif strcmp(dataset,'#7-Texas-1') == 1 % Heterogeneous CD of multispectral VS. multispectral
    ...
    subplot(133);imshow(Ref_gt,[]);title('Refgt')
elseif strcmp(dataset,'#7-Texas-2') == 1 % Heterogeneous CD of multispectral VS. multispectral
    load('Cross-sensor-Bastrop-data.mat')
    image_t1 = double(t1_L5);
    image_t2 = double(t2_L8);
    gt = double(ROI_2);
    Ref_gt = gt(:,:,1);
    opt.type_t1 = 'optical';
    opt.type_t2 = 'optical';
    figure;
    subplot(131);imshow(2*uint8(image_t1(:,:,[4 3 2])));title('imaget1')
    subplot(132);imshow(2*uint16(image_t2(:,:,[5 4 3])));title('imaget2')
    subplot(133);imshow(Ref_gt,[]);title('Refgt')
end
image_t1(find(isnan(image_t1)==1)) = 0;
image_t2(find(isnan(image_t2)==1)) = 0;
